...
. 커넥터를 배포할 때 를 선택하라는 메시지가 나타납니다 https://["서비스 계정"^] VM 인스턴스의 경우. BlueXP는 서비스 계정에서 권한을 받아 사용자를 대신하여 Cloud Volumes ONTAP 시스템을 생성 및 관리하는 등의 작업을 수행합니다. 권한은 서비스 계정에 사용자 지정 역할을 첨부하여 제공됩니다.
...
* https://["Google Cloud에서 Cloud Volumes ONTAP를 배포하고 프로젝트를 선택하는 방법에 대해 알아보십시오"^]
