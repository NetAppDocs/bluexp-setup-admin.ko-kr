...
각 배포 모드는 아웃바운드 연결 요구 사항, 배포 위치, 설치 프로세스, 인증 방법, 사용 가능한 데이터 및 저장 서비스, 청구 방법 측면에서 다릅니다.

표준 모드:: 웹 기반 콘솔에서 SaaS 서비스를 이용하세요. 사용하려는 데이터 서비스 및 기능에 따라 BlueXP 관리자는 하이브리드 클라우드 환경의 데이터를 관리하기 위해 하나 이상의 커넥터를 생성합니다.
+
--
이 모드는 공용 인터넷을 통한 암호화된 데이터 전송을 사용합니다.

--
제한된 모드:: 클라우드(정부 지역, 주권 클라우드 지역 또는 상업 지역)에 BlueXP 커넥터를 설치하고 BlueXP SaaS 계층에 대한 아웃바운드 연결을 제한합니다. 사용자는 SaaS 계층이 아닌 Connector에서 사용할 수 있는 웹 기반 콘솔에서 로컬로 BlueXP에 액세스합니다.
+
--
...
--
비공개 모드:: 온프레미스 또는 클라우드(보안 지역, 주권 클라우드 지역 또는 상업 지역)에 BlueXP 커넥터를 설치하면 BlueXP SaaS 계층에 연결할 수 없습니다. 사용자는 SaaS 계층이 아닌 Connector에서 사용할 수 있는 웹 기반 콘솔에서 로컬로 BlueXP에 액세스합니다.
+
--
