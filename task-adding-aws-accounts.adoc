...
NOTE: BlueXP에서 Connector를 배포하면 Connector를 배포한 계정에 대한 AWS 자격 증명이 BlueXP에 자동으로 추가됩니다. 기존 시스템에 Connector 소프트웨어를 수동으로 설치한 경우에는 이 초기 계정이 추가되지 않습니다. link:concept-accounts-aws.html["AWS 자격 증명 및 권한에 대해 알아보십시오"].
