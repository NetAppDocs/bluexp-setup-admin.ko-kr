...
image:screenshot-account-selection.png["BlueXP를 생성하려는 NetApp 계정을 묻는 BlueXP 설치 마법사 스크린샷"]
...
BlueXP의 계정 관리 위젯을 통해 계정 관리자가 NetApp 계정을 관리할 수 있습니다. 방금 계정을 만든 경우 처음부터 다시 시작할 수 있습니다. 그러나 이미 계정을 설정한 경우에는 계정과 연결된 사용자, 작업 영역 및 커넥터가 _ALL_으로 표시됩니다.
...
Connector를 두 개 이상의 NetApp 클라우드 데이터 서비스와 함께 사용할 수 있습니다. 예를 들어, Connector를 사용하여 Cloud Volumes ONTAP를 관리하는 경우 Cloud Tiering과 같은 다른 서비스와 동일한 Connector를 사용할 수 있습니다.
