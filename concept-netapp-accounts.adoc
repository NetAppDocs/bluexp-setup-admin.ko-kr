...
[role="lead"]
제한 모드 또는 비공개 모드에서 BlueXP 를 사용하는 경우 _BlueXP  계정 _ 을(를) 사용하여 사용자를 관리하고 isolated_workspace_에서 리소스를 구성할 수 있습니다. 예를 들어 사용자 그룹은 다른 작업 영역에서 작업 환경을 관리하는 사용자에게 표시되지 않는 작업 영역에서 Cloud Volumes ONTAP 작업 환경을 배포하고 관리할 수 있습니다.
