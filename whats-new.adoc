...
image:screenshot-edit-name.png["사용자 설정에서 사용자 이름을 편집하는 기능을 보여 주는 스크린샷."]

페더레이션 연결 또는 NetApp Support 사이트 계정으로 로그인한 경우에는 사용자 이름 편집이 지원되지 않습니다.



=== Amazon EC2 인스턴스 메타데이터 서비스 요구 사항

AWS에 Connector 소프트웨어를 수동으로 설치하기 전에 EC2 인스턴스에 IMDSv1이 필요하다는 것을 알고 있어야 합니다.

link:reference-limitations.html["AWS에서 IMDSv1의 요구 사항에 대해 자세히 알아보십시오"].
