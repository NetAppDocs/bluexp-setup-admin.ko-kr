...
Docker Engine은 Ubuntu 22.04 LTS에 커넥터를 설치할 때 필요한 컨테이너 오케스트레이션 도구입니다.

Connector의 3.9.44 릴리즈부터 Docker Engine 26이 _NEW_Connector 설치와 함께 지원됩니다.

지원되는 Docker Engine 버전의 업데이트된 목록은 다음과 같습니다.
