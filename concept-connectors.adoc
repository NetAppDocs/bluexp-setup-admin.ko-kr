...
+
https://["온프레미스 ONTAP 클러스터를 위한 검색 및 관리 옵션에 대해 자세히 알아보십시오"^]
...
+
Azure의 커넥터는 해당 커넥터가 관리하는 Cloud Volumes ONTAP 시스템과 동일한 Azure 영역에 배포하거나 에 배포되어야 합니다 https://["Azure 지역 쌍"^] Cloud Volumes ONTAP 시스템의 경우 이 요구 사항은 Cloud Volumes ONTAP와 연결된 스토리지 계정 간에 Azure 전용 링크 연결이 사용되도록 합니다. https://["Cloud Volumes ONTAP에서 Azure 프라이빗 링크를 사용하는 방법에 대해 알아보십시오"^]
...
* link:concept-modes.html["BlueXP 배포 모드에 대해 알아보십시오"]
* link:task-quick-start-standard-mode.html["표준 모드에서 BlueXP를 빠르게 시작합니다"]
* link:task-quick-start-restricted-mode.html["제한된 모드에서 BlueXP를 빠르게 시작합니다"]
* link:task-quick-start-private-mode.html["비공개 모드에서 BlueXP를 빠르게 시작합니다"]
...
** link:task-set-up-permissions-google.html["Google Cloud 권한을 설정합니다"]
...
Connector에 필요한 정확한 권한을 보려면 다음 페이지를 참조하십시오.
...
Connector는 아웃바운드 인터넷 액세스를 통해 소프트웨어 업데이트를 받는 경우 소프트웨어를 최신 버전으로 자동 업데이트합니다. 비공개 모드에서 BlueXP를 사용하는 경우 커넥터를 수동으로 업그레이드해야 합니다.

link:task-managing-connectors.html["Connector 소프트웨어를 수동으로 업그레이드하는 방법에 대해 알아보십시오"].
...
. 기존 작업 환경을 파악합니다.
+
** https://["기존 Cloud Volumes ONTAP 시스템을 BlueXP에 추가합니다"^]
** https://["ONTAP 클러스터에 대해 알아보십시오"^]


. 를 설정합니다 https://["용량 관리 모드"^]
+
