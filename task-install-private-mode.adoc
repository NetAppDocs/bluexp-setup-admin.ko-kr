...
summary: 비공개 모드에서 BlueXP를 설정하려면 Connector를 설치하고 Connector에서 실행 중인 사용자 인터페이스에 액세스하여 BlueXP를 설정한 다음 클라우드 권한을 제공해야 합니다. 
---
= 커넥터를 비공개 모드로 배포합니다
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
비공개 모드에서 BlueXP를 설정하려면 Connector를 설치하고 Connector에서 실행 중인 사용자 인터페이스에 액세스하여 BlueXP를 설정한 다음 클라우드 권한을 제공해야 합니다.
...
에서 BlueXP를 사용하려는 경우 https://aws.amazon.com/federal/us-intelligence-community/["AWS C2S(Commercial Cloud Services) 환경"^] 그런 다음 별도의 지침에 따라 해당 환경에서 시작해야 합니다. https://docs.netapp.com/us-en/cloud-manager-cloud-volumes-ontap/task-getting-started-aws-c2s.html["AWS C2S 환경에서 Cloud Volumes ONTAP를 시작하는 방법에 대해 알아보십시오"^]
