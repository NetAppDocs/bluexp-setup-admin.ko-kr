...
.시작하기 전에
BlueXP  커넥터를 설정하는 사용자는 표준 모드로 운영되는 BlueXP  조직에 속하지 않는 로그인을 사용하여 BlueXP 에 로그인해야 합니다.

BlueXP  로그인이 표준 모드로 운영되는 조직과 연결되어 있는 경우 조직에 속하지 않는 새 BlueXP  로그인 또는 NetApp 지원 사이트 자격 증명을 사용하여 등록해야 합니다. 그렇지 않으면 제한 모드를 활성화하는 옵션이 설정 화면에 표시되지 않습니다.
