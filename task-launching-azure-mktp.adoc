...
[role="lead"]
Azure 상용 지역의 경우 Cloud Manager에서 직접 Connector를 생성하는 것이 가장 좋지만, 원하는 경우 Azure Marketplace에서 Connector를 실행할 수 있습니다. Azure Government 지역의 경우 Cloud Manager의 SaaS 웹 사이트에서 Connector를 정부 지역에 배포할 수 없으므로, Azure 마켓플레이스에서 이 옵션을 선택하는 것이 좋습니다.
...
Connector가 Azure 상용 지역에 있는 경우 웹 브라우저를 열고 로 이동합니다 https://cloudmanager.netapp.com[] Connector with Cloud Manager 사용을 시작합니다.

Connector가 Azure Government 지역에 있는 경우 웹 브라우저를 열고 Connector 인스턴스의 IP 주소에 연결하여 Cloud Manager를 사용할 수 있습니다. http://_ipaddress_:80[]
