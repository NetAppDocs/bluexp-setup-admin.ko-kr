...
[role="lead"]
Azure에서 실행되는 Linux 호스트에 Connector를 수동으로 설치할 수 있습니다. 자신의 Linux 호스트에 Connector를 수동으로 설치하려면 호스트 요구 사항을 검토하고 네트워킹을 설정하고 Azure 사용 권한을 준비하고 커넥터를 설치한 다음 준비한 사용 권한을 제공해야 합니다.
