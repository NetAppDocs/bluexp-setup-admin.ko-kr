...
마켓플레이스 가입에서는 다음 BlueXP  데이터 서비스에 대한 요금을 부과할 수 있습니다.
...
.시작하기 전에
BlueXP  데이터 서비스를 구독하려면 마켓플레이스 가입을 Connector와 연결된 클라우드 자격 증명과 연결해야 합니다. "표준 모드 시작" 워크플로를 따랐다면 이미 Connector가 있는 것입니다. 자세한 내용은 를 link:task-quick-start-standard-mode.html["표준 모드에서 BlueXP를 빠르게 시작합니다"]참조하십시오.
