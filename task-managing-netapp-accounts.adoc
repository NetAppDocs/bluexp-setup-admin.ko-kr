...
[role="lead"]
link:task-setting-up-netapp-accounts.html["초기 설정을 수행한 후"]나중에 사용자, 서비스 계정, 작업 영역 및 커넥터를 관리하여 계정 설정을 관리할 수 있습니다.
...
* 클라우드 백업
+
SaaS 플랫폼이 비활성화된 경우 정부 지역에서는 Cloud Backup이 지원되지만, SaaS 플랫폼이 비활성화된 경우에는 상업 지역에서는 지원되지 않습니다
