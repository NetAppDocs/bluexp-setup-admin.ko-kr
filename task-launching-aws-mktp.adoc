...
[role="lead"]
AWS 상업 지역의 경우 Cloud Manager에서 직접 Connector를 생성하는 것이 가장 좋지만, 원하는 경우 AWS Marketplace에서 Connector를 실행할 수 있습니다. AWS 정부 지역의 경우 Cloud Manager의 SaaS 웹 사이트에서 정부 지역에 Connector를 구축할 수 없으므로 AWS Marketplace에서 다음 번에 선택하는 것이 좋습니다.
...
Cloud Manager용 AWS Marketplace 오퍼링을 통해 AWS 상업 지역에서 Connector 인스턴스를 직접 실행할 수 있습니다.
...
. 로 이동합니다 https://aws.amazon.com/marketplace/pp/B018REK8QG["Cloud Manager 페이지로 이동하여 AWS 마켓플레이스를 확인하십시오"^] AMI에서 Cloud Manager를 구축하려면 다음을 따르십시오.
...
** * 응용 프로그램 및 OS 이미지 *: 이 섹션을 건너뜁니다. Cloud Manager AMI가 이미 선택되었습니다.
...
** * 고급 세부 정보 *: * IAM intance profile * 에서 1단계에서 만든 IAM 역할을 선택합니다.
...
. 웹 브라우저를 열고 로 이동합니다 https://cloudmanager.netapp.com[] Connector with Cloud Manager 사용을 시작합니다.
...
AWS 정부 지역에서 Connector를 구축하려면 EC2 서비스로 이동하여 AWS Marketplace에서 Cloud Manager 오퍼링을 선택해야 합니다.
...
. AWS Marketplace에서 Cloud Manager 제품으로 이동합니다.
+
...
.. Cloud Manager를 검색하고 오퍼링을 선택합니다.
+
image:screenshot-gov-cloud-mktp.png["AWS 마켓플레이스에서 검색한 후 Cloud Manager 오퍼링을 보여주는 스크린샷"]
...
Cloud Manager를 사용하려는 경우 언제든지 웹 브라우저를 열고 Connector 인스턴스의 IP 주소에 연결합니다. http://_ipaddress_:80[]
