...
[role="lead"]
BlueXP 계정 관리자는 대부분의 BlueXP 기능을 사용하기 전에 _Connector_를 배포해야 합니다. BlueXP는 Connector를 사용하여 퍼블릭 클라우드 환경 내의 리소스와 프로세스를 관리할 수 있습니다. link:concept-connectors.html["커넥터가 필요한 시기를 알아보십시오"].

이 페이지에서는 BlueXP에서 직접 Azure에서 커넥터를 생성하는 방법에 대해 설명합니다. link:concept-connectors.html#how-to-create-a-connector["커넥터를 배포하는 다른 방법에 대해 알아봅니다"].
...
Connector를 배포하려면 Azure에서 Connector VM을 생성하는 데 필요한 권한이 있는 로그인을 BlueXP에 제공해야 합니다.

두 가지 옵션이 있습니다.

. 메시지가 나타나면 Microsoft 계정으로 로그인합니다. 이 계정에는 특정 Azure 권한이 있어야 합니다. 이 옵션이 기본 옵션입니다.
+
<<Azure 계정을 사용하여 커넥터를 만듭니다,시작하려면 아래 단계를 따르십시오>>.

. Azure AD 서비스 보안 주체에 대한 세부 정보를 제공합니다. 이 서비스 보안 주체는 특정 권한도 필요합니다.
+
<<서비스 보안 주체를 사용하여 커넥터를 만듭니다,시작하려면 아래 단계를 따르십시오>>.
...
BlueXP에서 Connector를 배포하기 전에 Azure 계정에 올바른 권한이 있는지 확인해야 합니다.
...
. BlueXP에서 커넥터를 배포할 사용자에게 역할을 할당합니다.
+
...
BlueXP를 사용하면 사용자 인터페이스에서 직접 Azure에 Connector를 생성할 수 있습니다.
...
.작업을 마친 후
작업 영역 관리자가 이러한 커넥터를 사용하여 Cloud Volumes ONTAP 시스템을 만들 수 있도록 작업 영역과 커넥터를 연결해야 합니다. Account Admins만 있는 경우에는 Connector를 작업 영역과 연결할 필요가 없습니다. 계정 관리자는 기본적으로 BlueXP의 모든 작업 영역에 액세스할 수 있습니다. link:task-setting-up-netapp-accounts.html#associating-connectors-with-workspaces["자세한 정보"].
...
Azure 계정으로 로그인하는 대신 필요한 권한이 있는 Azure 서비스 보안 주체에 대한 자격 증명을 BlueXP에 제공할 수도 있습니다.



=== 서비스 보안 주체를 사용하여 Azure 사용 권한 부여

Azure Active Directory에서 서비스 보안 사용자를 생성 및 설정하고 BlueXP에 필요한 Azure 자격 증명을 획득하여 Azure에 Connector를 배포하는 데 필요한 권한을 부여합니다.
...
BlueXP에서 Connector를 배포하는 데 사용할 수 있는 Azure AD(Active Directory) 응용 프로그램 및 서비스 보안 주체를 만듭니다.
...
BlueXP에서 커넥터를 생성할 때 응용 프로그램의 응용 프로그램(클라이언트) ID와 디렉터리(테넌트) ID를 제공해야 합니다. BlueXP는 ID를 사용하여 프로그래밍 방식으로 로그인합니다.
